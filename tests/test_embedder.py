...
@patch("notes_search.utils.embedder.httpx.post")
def test_returns_one_embedding_per_text(mock_post):
    mock_post.return_value = _mock_response([0.1, 0.2, 0.3])
    result = embed("http://localhost:11434", "nomic-embed-text", ["hello", "world"])
...
def test_passes_timeout(mock_post):
    ...
    embed("http://localhost:11434", "nomic-embed-text", ["text"])
