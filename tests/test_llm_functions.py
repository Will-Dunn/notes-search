...
@patch("notes_search.adapters.ollama_tagger.httpx.post")
def test_valid_json_returns_lowercase_tags(mock_post):
    mock_post.return_value = _mock_response('["Python", "Machine-Learning"]')
    tags = OllamaTagger("http://localhost:11434", "llama3.1:8b").tag("some note content")
...
def test_passes_timeout(mock_post):
    ...
    OllamaTagger("http://localhost:11434", "llama3.1:8b").tag("content")
...
def test_empty_json_array_returns_empty_list(mock_post):
    mock_post.return_value = _mock_response("[]")
    assert OllamaTagger("http://localhost:11434", "llama3.1:8b").tag("content") == []
